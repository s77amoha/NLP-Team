...
# Train SVM classifier for language
svm_lang = LinearSVC()
svm_lang.fit(X_train, y_train_lang)

# Train SVM classifier for narrative labels
svm_narrative = OneVsRestClassifier(LinearSVC())
svm_narrative.fit(X_train, y_train_narrative)

# Train SVM classifier for subnarrative labels
svm_subnarrative = OneVsRestClassifier(LinearSVC())
svm_subnarrative.fit(X_train, y_train_subnarrative)

# Predict on testing data
pred_lang = svm_lang.predict(X_test)
pred_narrative = svm_narrative.predict(X_test)
pred_subnarrative = svm_subnarrative.predict(X_test)
...
def plot_misclassified_examples(test_titles, test_texts, test_labels_narrative, pred_labels_narrative, test_labels_subnarrative, pred_labels_subnarrative, test_languages, pred_labels_lang):
    ...
    plt.title('Misclassified Examples of SVM Classifier')
    plt.show()
